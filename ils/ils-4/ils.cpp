...
void merge(char *a, int idx, double r) {
	vector<int> p;
	for(int i = 0; i < m; ++i) {
		if(a[i] != word[i * l + idx]) p.emplace_back(i);
	}
	shuffle(p.begin(), p.end(), rng);
	int n = ceil(r * int(size(p)));
	for(int i = 0; i < n; ++i) a[p[i]] = word[p[i] * l + idx];
}
...
void solucao_inicial(char *s) {
	...
	shuffle(P.begin(), P.end(), rng);
	for(int j = 0; j < m; ++j) s[j] = word[j * l];
	for(int i = 1; i < n; ++i) merge(s, P[i], 1.0 / (i + 1));
}
